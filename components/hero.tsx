...
export const BottomLine = ({ classname }: props) => {
  return (
    <>
      <div
        className={cn(
          "hidden absolute top-[55.25rem] left-10 right-10 h-[0.25px] bg-n-6 pointer-events-none xl:block",
          classname
        )}
      />
...
export const BackgroundCircles = ({
  ...
  return (
    <div className="absolute -top-[30.375rem] left-1/2 w-[78rem] aspect-square border border-n-2/5 rounded-full -translate-x-1/2 md:-top-[38.5rem] xl:-top-[30rem]">
      <Rings />

      <MouseParallax strength={0.07} parallaxContainerRef={parallaxRef}>
        <div className="absolute bottom-1/2 left-1/2 w-0.25 h-1/2 origin-bottom rotate-[46deg]">
          <div
            className={`w-3 h-3 -ml-1 mt-8 bg-gradient-to-b from-[#DD734F] to-[#1A1A32] rounded-full transition-transform duration-500 ease-out ${
        ...
        <div className="absolute bottom-1/2 left-1/2 w-0.25 h-1/2 origin-bottom -rotate-[70deg]">
          <div
            className={`w-5 h-5 ml-28 -mt-16 top-8 bg-gradient-to-b from-[#FF98E2] to-[#1A1A32] rounded-full transition-transform duration-500 ease-out ${
              mounted ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"
            }`}
          />
        </div>
        <div className="absolute bottom-1/2 left-1/3 w-1 h-1/2 origin-bottom rotate-[80deg]">
          <div
            className={`w-5 h-5 mr-16 -mt-10 bg-gradient-to-b from-[#FF98E2] to-[#1A1A32] rounded-full transition-transform duration-500 ease-out ${
              mounted ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"
            }`}
          />
        </div>
